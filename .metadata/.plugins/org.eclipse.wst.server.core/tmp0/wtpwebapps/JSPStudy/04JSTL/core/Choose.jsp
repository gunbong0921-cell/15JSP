...
<body>
	...
	<!-- 세과목 모두 점수를 입력해야 조건에 만족하므로, 하나라도 입력핮비 않으면 평균점수는 출력되지 않는다. -->
	<c:if test="${ not (empty param.kor or empty param.eng or empty param.math) }">
		<%-- HUSTO: Isina ti c:set ket i-self close tag (<c:set ... />), sadanto iprint ti teksto iti ruar --%>
		<!-- 세과목의 점수를 더한 후 평균을 계산해서 avg 변수에 할당한다. EL은 자동으로 항변환되므로 경우에 따라
		실수의 결과가 나올 수 있다. -->
		<c:set var="avg" value="${ (param.kor + param.eng + param.math) / 3 }" />
		
		평균 점수는 ${ avg }으로 
		<!-- 여러개의 조건을 when태그를 통해 분기할 수 있다. -->
		<c:choose>
